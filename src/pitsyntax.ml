...
let get_pred env (c, ext) tl =
  try
	...
	if not ((List.length r.p_type == List.length tl) && (List.for_all2 (fun t1 t2 -> t1 == t2 || t1 == Param.any_type) r.p_type tl)) then
	  input_error ("predicate " ^ c ^ " expects " ^ 
...
let get_pred_or_fun env (c,ext) tl' =
  try
    ...
    | EFun r ->
	if not (Terms.eq_lists (fst r.f_type) tl') then
	  input_error ("function " ^ c ^ " expects " ^ 
		       (args_to_string (fst r.f_type)) ^
		       " but is here given " ^
		       (args_to_string tl')) ext;
...
let rec copy_process add_in_glob_table = function
  ...
  | Test(t,t',p,q,occ) -> 
      ...
      let p' = copy_process add_in_glob_table p in
      let q' = copy_process add_in_glob_table q in
      Test(Terms.copy_term3 t, Terms.copy_term3 t', p', q', occ')
...
let rec check_term env (term, ext) =
  ...
  | PPFunApp((s,ext),l) -> 
	  ...
	  | _ -> 
	      input_error ("function choice expects two arguments of same type but is here given " ^
			   (args_to_string tl')) ext
        end
      else
        begin
  ...
  | PPRestr((s,ext),tyid,t) -> 
	...
	input_warning ("identifier " ^ s ^ " rebound") ext;
      ...
      ((fun l -> Restr(r, fex l, new_occurrence())), tex, ty)
  | PPTest(c,p1,p2) -> 
      let rec interpret_cond p1 p2 = function
	  (PPIdent pred), ext -> interpret_cond p1 p2 (PPFunApp(pred,[]), ext) 
	| (PPTuple _), ext ->
	    input_error "a tuple is not a condition (conditions must have type bool)" ext
	| (PPRestr _ | PPTest _ | PPLetIn _ | PPLet _ | PPLetFilter _), ext -> 
	    let (fex, lex', ty) = check_term env c in
	    if ty != Param.bool_type then
	      input_error ("term has type " ^ ty.tname ^ " but a boolean is expected") ext;
...
let rec check_process env = function 
  ...
  | PRestr((s,ext),t,p) -> 
	...
	input_warning ("identifier " ^ s ^ " rebound") ext;
      let r = create_name s (Param.tmp_type, ty) false in
      Restr(r, check_process (StringMap.add s (EName r) env) p, new_occurrence())
  | PInput(tc,pat,p) ->
	...
	match StringMap.find i env with
	  ETable f -> 
		...
		Some t -> 
	      ...
	      |	None -> 
		  fex' (List.map (fun l' -> 
		    Get(PatTuple(f, l'), FunApp(true_cst, []), check_process env' p, 
			new_occurrence())) lex')		  
	    end
	| _ ->
...
let rec rename_process = function
  ...
  | PGet(i,patl,topt,p) -> PGet(rename_ie i ,List.map rename_pat patl, (match topt with None -> None | Some t -> Some (rename_pterm t)), rename_process p)
...
let parse_file s = 
  ...
  let p = Terms.auto_cleanup (fun () ->
    let p' = check_process (!global_env) proc in
