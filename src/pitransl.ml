...
let record_eqs () =
  if TermsEq.hasEquationsToRecord() then
	...
	begin
      ...
      TermsEq.record_eqs();
      if !Param.html_output then
	Display.Html.print_string "<H2>Completed destructors</H2>\n"
	...
	match f.f_cat with
	  Red red_rules -> 
	    ...
	    if !Param.html_output then
	      Display.Html.display_red f red_rules'
	    else if !Param.verbose_eq then
	      Display.Text.display_red f red_rules';
...
let rec move_new accu = function
  ...
  | Test(t1,t2,p1,p2,occ) -> 
	...
	let (l1,l2) = list_split (fun (f,_) -> occurs_term f t1 || occurs_term f t2) accu in
        put_new l1 (Test(t1,t2,move_new l2 p1,Nil,occ))
  | Input(t,pat,p,occ) ->
      let (l1,l2) = list_split (fun (f,_) -> occurs_term f t || occurs_pat f pat) accu in
  ...
  | LetFilter(vl,fact,p,q,occ) ->
	...
	let (l1,l2) = list_split (fun (f,_) -> occurs_fact f fact) accu in
	put_new l1 (LetFilter(vl, fact, move_new l2 p,Nil,occ))
  | Event(t1,p,occ) ->
      let (l1,l2) = list_split (fun (f,_) -> occurs_term f t1) accu in
      put_new l1 (Event(t1, move_new l2 p, occ))
  | Insert(t1,p,occ) ->
      let (l1,l2) = list_split (fun (f,_) -> occurs_term f t1) accu in
      put_new l1 (Insert(t1, move_new l2 p, occ))
  | Get(pat,t1,p,occ) ->
      let (l1,l2) = list_split (fun (f,_) -> occurs_term f t1 || occurs_pat f pat) accu in
